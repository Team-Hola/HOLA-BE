...
export class CampaignsService {
  ...
  //캠페인의 광고 리스트 조회
  async getAdsByCampaignId(campaignId: Types.ObjectId) {
    return await this.advertisementsService.getAdvertisementInCampaign(campaignId);
  }

  // // 광고 결과 집계
  async getCampaignResult(campaignId: Types.ObjectId) {
    const campaign = await this.campaignsRepository.findByCampaignId(campaignId);
    const adList = await this.advertisementsService.getAdvertisementInCampaign(campaignId);
    const adIdList = adList.map((ad: any) => {
      return new Types.ObjectId(ad._id);
    });
    // 로그 집계
    const aggregate = await this.advertisementLogsService.getAdResult(adIdList);
